                    ...
                    <div class="breadcrumb_text">
                        ...
                        <ul>
                            ...
                            @if ($menuItem->category)
                                <li><a
                                        href="{{ route('website.menu', ['category' => $menuItem->category->slug]) }}">{{ $menuItem->category->name }}</a>
                                </li>
                            @endif
                ...
                <div class="col-xl-5 col-md-8 col-lg-7 wow fadeInUp">
                    <div class="menu_det_text">
                        <div class="menu_det_text_area d-flex align-items-center gap-3 flex-wrap mb-2">
                            ...
                            @if ($menuItem->category)
                                <a href="{{ route('website.menu', ['category' => $menuItem->category->slug]) }}"
                        ...
                        <div class="dietary_info mb-3">
                            @if ($menuItem->is_vegetarian)
                                <span class="badge bg-success me-1"><i
                                        class="fas fa-leaf me-1"></i>{{ __('Vegetarian') }}</span>
                            @endif
                            @if ($menuItem->is_vegan)
                                <span class="badge bg-success me-1"><i
                                        class="fas fa-seedling me-1"></i>{{ __('Vegan') }}</span>
                            @endif
                            @if ($menuItem->is_spicy)
                                <span class="badge bg-danger me-1"><i
                                        class="fas fa-pepper-hot me-1"></i>{{ __('Spicy') }}</span>
                            @endif
                            @if ($menuItem->calories)
                                <span class="badge bg-info me-1"><i class="fas fa-fire me-1"></i>{{ $menuItem->calories }}
                                    {{ __('cal') }}</span>
                            @endif
                            @if ($menuItem->preparation_time)
                                <span class="badge bg-warning text-dark"><i
                                        class="fas fa-clock me-1"></i>{{ $menuItem->preparation_time }}
                                    {{ __('min') }}</span>
                            @endif
                        </div>
                        ...
                        <div class="special_instructions mb-3">
                            <label class="form-label">{{ __('Special Instructions') }} <small
                                    class="text-muted">({{ __('optional') }})</small></label>
                            <textarea id="specialInstructions" rows="2" placeholder="{{ __('Any special requests or dietary notes...') }}"
                        ...
                        <div class="details_cart_btn">
                            ...
                            <button type="button" class="common_btn" id="buyNowBtn"
                                data-item-id="{{ $menuItem->id }}">
                                {{ __('Buy Now') }}
                            </button>
                            <a class="love favorite-btn {{ session('favorites.' . $menuItem->id) ? 'active' : '' }}"
                                href="#" data-item-id="{{ $menuItem->id }}">
                        ...
                        <ul class="share">
                            ...
                            <li><a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($menuItem->name) }}"
                                    target="_blank"><i class="fab fa-twitter"></i></a></li>
                            <li><a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode(request()->url()) }}"
                                    target="_blank"><i class="fab fa-linkedin-in"></i></a></li>
                            <li><a href="https://wa.me/?text={{ urlencode($menuItem->name . ' - ' . request()->url()) }}"
                    ...
                    <div class="">
                        ...
                        @if ($menuItem->allergens && count($menuItem->allergens) > 0)
                            <div class="menu_details_offer mb-3">
                                <p><strong><i
                                            class="fas fa-exclamation-triangle text-warning me-2"></i>{{ __('Allergens') }}</strong>
                                </p>
                                ...
                                <div class="menu_det_description">
                                    ...
                                    @if ($menuItem->calories || $menuItem->preparation_time)
                                        <div class="nutritional_info mt-4">
                                            ...
                                            <ul>
                                                @if ($menuItem->calories)
                                                    <li><strong>{{ __('Calories') }}:</strong> {{ $menuItem->calories }}
                                                        kcal</li>
                                                @endif
                                                @if ($menuItem->preparation_time)
                                                    <li><strong>{{ __('Preparation Time') }}:</strong>
                                                        {{ $menuItem->preparation_time }} {{ __('minutes') }}</li>
                                                @endif
                                            </ul>
                                        </div>
                                    @endif
                            ...
                            <div class="single_menu">
                                <div class="single_menu_img">
                                    ...
                                    <ul>
                                        ...
                                        <li><a href="#" class="favorite-btn" data-item-id="{{ $item->id }}"><i
