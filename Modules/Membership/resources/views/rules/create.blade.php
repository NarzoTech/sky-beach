...
@section('content')
    <div class="card mb-5">
        <div class="card-header">
            <h4 class="section_title">{{ __('Create Rule for') }}: {{ $program->name }}</h4>
        </div>
        <div class="card-body">
            <form action="{{ route('membership.rules.store') }}" method="POST">
                @csrf
                <input type="hidden" name="loyalty_program_id" value="{{ $program->id }}">
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="priority" id="priority" class="form-control" value="{{ old('priority', 0) }}" min="0" max="1000">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="condition_type" id="condition_type" class="form-control" required>
                                <option value="category" {{ old('condition_type') == 'category' ? 'selected' : '' }}>{{ __('Category') }}</option>
                                <option value="item" {{ old('condition_type') == 'item' ? 'selected' : '' }}>{{ __('Item') }}</option>
                                <option value="amount" {{ old('condition_type') == 'amount' ? 'selected' : '' }}>{{ __('Amount') }}</option>
                                <option value="time_period" {{ old('condition_type') == 'time_period' ? 'selected' : '' }}>{{ __('Time Period') }}</option>
                                <option value="customer_group" {{ old('condition_type') == 'customer_group' ? 'selected' : '' }}>{{ __('Customer Group') }}</option>
                            </select>
                    ...
                    <div class="col-md-6 condition-field" id="condition_amount_field" style="display: none;">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" name="condition_min_amount" id="condition_min_amount" class="form-control" value="{{ old('condition_min_amount') }}">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="action_type" id="action_type" class="form-control" required>
                                <option value="earn_points" {{ old('action_type') == 'earn_points' ? 'selected' : '' }}>{{ __('Earn Points') }}</option>
                                <option value="bonus_points" {{ old('action_type') == 'bonus_points' ? 'selected' : '' }}>{{ __('Bonus Points') }}</option>
                                <option value="multiply_points" {{ old('action_type') == 'multiply_points' ? 'selected' : '' }}>{{ __('Multiply Points') }}</option>
                                <option value="redeem_discount" {{ old('action_type') == 'redeem_discount' ? 'selected' : '' }}>{{ __('Redeem Discount') }}</option>
                            </select>
                            <small class="text-muted" id="action_type_help">{{ __('Earn: Override base rate | Bonus: Add extra points | Multiply: 2x, 3x points | Redeem: Special discount') }}</small>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="action_value">{{ __('Action Value') }} <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" name="action_value" id="action_value" class="form-control" value="{{ old('action_value') }}" required>
                            <small class="text-muted" id="action_value_help">{{ __('Points to earn/add, or multiplier value (e.g., 2 for double points)') }}</small>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="applies_to" id="applies_to" class="form-control" required>
                                <option value="all" {{ old('applies_to') == 'all' ? 'selected' : '' }}>{{ __('All') }}</option>
                                <option value="specific_items" {{ old('applies_to') == 'specific_items' ? 'selected' : '' }}>{{ __('Specific Items') }}</option>
                                <option value="specific_categories" {{ old('applies_to') == 'specific_categories' ? 'selected' : '' }}>{{ __('Specific Categories') }}</option>
                                <option value="specific_customers" {{ old('applies_to') == 'specific_customers' ? 'selected' : '' }}>{{ __('Specific Customers') }}</option>
                            </select>
                ...
                <div class="mt-3">
                    <a href="{{ route('membership.rules.index', ['program_id' => $program->id]) }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
