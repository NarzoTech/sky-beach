...
@section('title', __('Edit Loyalty Program'))
@section('content')
    @php
        $earningRules = $program->earning_rules ?? [];
        $spendAmount = $earningRules['spend_amount'] ?? '';
        $pointsEarned = $earningRules['points_earned'] ?? '';
    ...
    <form action="{{ route('membership.programs.update', $program) }}" method="POST">
        ...
        <div class="card mb-4">
            <div class="card-header">
                <h4 class="section_title">{{ __('Edit Program') }}: {{ $program->name }}</h4>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="description" id="description" class="form-control" value="{{ old('description', $program->description) }}" placeholder="{{ __('Optional description') }}">
                            @error('description')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="row align-items-end">
                    <div class="col-md-4">
                        <div class="form-group mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" min="1" name="spend_amount" id="spend_amount" class="form-control" value="{{ old('spend_amount', $spendAmount) }}" required placeholder="100">
                    ...
                    <div class="col-md-4">
                        <div class="form-group mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" step="1" min="1" name="points_earned" id="points_earned" class="form-control" value="{{ old('points_earned', $pointsEarned) }}" required placeholder="1">
                ...
                <div class="table-responsive">
                    <table class="table" id="coupon-tiers-table">
                        ...
                        <tbody id="coupon-tiers-body">
                            @php
                                $tierPoints = old('tier_points', array_column($couponTiers, 'points_required'));
                                $tierDiscounts = old('tier_discounts', array_column($couponTiers, 'discount_amount'));
                            @endphp
                            @if(count($tierPoints) > 0)
                                @foreach($tierPoints as $i => $tp)
                                    <tr class="tier-row">
                                        <td>
                                            <div class="input-group">
                                        ...
                                        <td>
                                            <div class="input-group">
                                                <input type="number" name="tier_discounts[]" class="form-control" min="1" step="0.01" value="{{ $tierDiscounts[$i] ?? '' }}" required placeholder="500">
                                                <span class="input-group-text">{{ currency_icon() }}</span>
                                            </div>
                                        </td>
