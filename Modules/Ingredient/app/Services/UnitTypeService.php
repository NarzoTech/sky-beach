...
class UnitTypeService
{
     ...
     * @param float $quantity
     * @param int $fromUnitId
     * @param int $ingredientBaseUnitId
     * @return float
     */
    public function convertToIngredientBaseUnit($quantity, $fromUnitId, $ingredientBaseUnitId)
    {
        return UnitConverter::convertToProductBaseUnit($quantity, $fromUnitId, $ingredientBaseUnitId);
    }
    ...
    {
        $unit_type = UnitType::findOrFail($id);
        if (count($unit_type->ingredients) > 0) {
            return "not_possible";
        }
        $unit_type->delete();
